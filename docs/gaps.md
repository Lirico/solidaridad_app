...
| G-P1-06 | Entry mode ISO acorde al modo de captura | done | **Rama 3 (2026-11-08):** gateway DE22 dinámico (`entry_mode` "012"/"022", DE35 track2 para banda) + API `entry_mode`/`track2` en `CreateTransactionRequest`. `make check` OK en gateway y API. **Rama 4 (2026-11-08):** mobile `registerSale` envía `entry_mode` ("022" banda / "012" manual) y `track2` (si está disponible) en el payload. `WaitingForCardScreen` pasa `entry_mode: '022'` + `track2`; `SaleManualCardScreen` pasa `entry_mode: '012'`. `flutter analyze` OK. |
