...
Última revisión: 2026-09-22 (Cierre de Lote en mobile)
...
> conservó la compactación validada en el V660P (ícono 100→64, títulos con
> `maxLines`/elipsis, paddings y espaciados menores en `sale_status_content.dart`)
> aplicada sobre el patrón de esta rama (`AppHeader` 64dp + `AppSheetPanel`), que agrega
> `FittedBox` anti-desborde horizontal en las filas del ticket. `sale_status_screen.dart`
> y `void_result_screen.dart` quedan en la versión de esta rama (cabecera compacta +
> panel blanco a sangre completa). `flutter analyze` OK y `flutter test` OK (14 tests).

> ✅ **Último cambio (2026-08-24, fix de UI en pantalla de resultado):** en el V660P la
> pantalla de venta aprobada/anulación mostraba "Bottom overflowed by 79 pixels". Se
> corrigió compactando el layout sin scroll: header 180→120px, se quitó el
> `Transform.translate(0,-20)`, ícono 100→64, espaciados/paddings reducidos y títulos
> con `maxLines`+elipsis. Aplicado a `SaleStatusScreen` y `VoidResultScreen`; al
> integrarse con esta rama quedan sobre el `AppHeader`/`AppSheetPanel` nuevo.
> Ver fila G-P1-13.
...
| G-P2-10 | Cierre de lote real: resumen desde la API y cierre contra el procesador | open | **2026-09-22 (pantalla de Cierre de Lote):** mobile ya muestra el resumen del lote actual leyendo `GET /v1/transactions` y lo cierra **localmente** (`mobile/lib/features/batch_close/**`: el cierre es un corte en el terminal + una secuencia local del Nº de lote). **Falta el contrato real:** (1) la API no expone lote/cierre — `api/persistence/models/transaction.py` no tiene `lote`, `id_cierre` ni `marca_cierre` (esos campos existen solo en la base del procesador, `payment_processor/docker/mysql/01_schema.sql`) y no hay `POST /v1/batch/close`; (2) el gateway no implementa MTI `0500` (solo `0100`, `0200`/`0210` y reverso `0400`): `payment-gateway/infrastructure/iso/**`; (3) el procesador responde `00` ante un cierre con diferencia detectada (BN-12) y `00`/`19` según marca ante fallo (`payment_processor/legacy/bin/auth_thread.c:295-311`), así que un cierre descuadrado se reportaría como exitoso. **Datos del mockup sin fuente:** el **total en pesos** no es calculable porque `amount` es cantidad y no importe (decisión abierta **D-3** en `docs/errores-ventas.md`; el precio por kg solo existe en MySQL del procesador, `precio_kg_gas`), por eso la pantalla muestra el total en **kg**; el **peso por producto** (10/15/30/45 kg) está hardcodeado en `mobile/lib/features/batch_close/domain/batch_close_model.dart` (`batchKgPerUnit`) porque `packages/catalog` solo expone `label`/`unit` (y `GRANEL` es m³, que el mockup muestra como kg); el **Nº de lote** es una secuencia local de la sesión y el corte se pierde al cerrar la app (la app no tiene persistencia local). **Además:** falta el ticket de cierre (el `ReceiptPrinter` actual imprime ventas de a una). **Se numera G-P2-10** (y no G-P2-07) para no colisionar con el trabajo pendiente de otros gaps. Evidencia: `mobile/lib/features/batch_close/**`; tests `mobile/test/batch_close_{summary,repository,content}_test.dart`. |
