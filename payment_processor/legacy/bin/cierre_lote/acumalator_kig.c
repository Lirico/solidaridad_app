...
Cupon* get_consumo_vivo(MYSQL* con, char* card_number, char* cod_moneda, int flag)
{
    ...
    if (flag == USER_TYPE)
    {
    	sprintf(sql, "SELECT  id, terminal_date, importe, cod_moneda, tipo_mensaje, procode, ts_operacion FROM sgas_cup_trx "
                "WHERE "
                " (tipo_mensaje!='0500' AND tipo_mensaje!='0400') AND cod_moneda='%s' AND nro_tarjeta='%s' AND "
                "id in (SELECT DISTINCT id FROM sgas_cup_trx) "
		...
		sprintf(sql, "SELECT  id, terminal_date, importe, cod_moneda, tipo_mensaje, procode, ts_operacion FROM sgas_cup_trx "
                "WHERE  "
                " (tipo_mensaje!='0500' AND tipo_mensaje!='0400') AND cod_moneda='%s' AND cod_comercio='%s' AND "
                "id in (SELECT DISTINCT id FROM sgas_cup_trx) "
                "ORDER BY ts_operacion",
                cod_moneda, card_number
    	);
