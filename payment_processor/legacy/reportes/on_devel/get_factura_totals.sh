...
(fecha_tr between concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH) , '%Y-%m'), '-16') and
...
concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH) , '%Y-%m'), '-16') and  LAST_DAY( DATE_SUB(CURRENT_DATE(),
INTERVAL 1 month)) )  ) t1,
...
, '%Y-%m'), '-16') and  LAST_DAY( DATE_SUB(CURRENT_DATE(), INTERVAL 1 month)) ) ) t2,
(select count(*) from iso_pool where mtype='0400' and
(respcode_39!='' and respcode_39='00') and (datetime_trx between concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH)
, '%Y-%m'), '-16') and  LAST_DAY( DATE_SUB(CURRENT_DATE(), INTERVAL 1 month)) ) ) t3,
(select concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH) , '%Y-%m'), '-16') ) t4,
...
concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') )
and tipo_trans='CARGA MENSUAL') ts1,
(select count(*) from reporte_diaria where
(fecha_tr between concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-01') and
concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') )
...
concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') )) t1,
(select count(*) from iso_pool where mtype in ('0100', '0200') and
(respcode_39!='' and respcode_39!='00') and (datetime_trx between concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH)
, '%Y-%m'), '-01') and concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') )) t2,
(select count(*) from iso_pool where mtype='0400' and
(respcode_39!='' and respcode_39='00') and (datetime_trx between concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH)
, '%Y-%m'), '-01') and concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') )) t3,
(select concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-01') ) t4,
(select concat(DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 0 MONTH) , '%Y-%m'), '-15') ) t5
;" 2>/dev/null
