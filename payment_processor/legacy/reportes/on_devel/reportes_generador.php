...
function RepoGenerador($con, $periodo, $fecha)
{
	...
	$sql = "create table kigsolidario2.sgas_tmp (select * from kigsolidario2.sgas_trx where terminal_date = '".$fecha."' and procode='020000'); ".
               "insert into kigsolidario2.reporte_diaria (ciudad,barrio,id_tr,fecha_tr,hora_tr,apellido_nombre,nro_doc, ".
...
"trx.terminal_date, trx.terminal_time, usr.apellido_nombre,  usr.id_usuario, '".$periodo."' as periodo,  ".
...
"if ((trx.tipo_mensaje='0200' and trx.procode='200001'), 'CARGA MENSUAL', 'TRANS' ))) cid, ".
...
"WHERE s1.terminal_date = '".$fecha."' AND s1.terminal_time!='00:16:21'); drop table kigsolidario2.sgas_tmp;";

	$qret = mysqli_multi_query($con, $sql) or die("RepoGenerador() MYSQL: ".mysqli_error($con)."\n");
...
function RepoGeneradorComer($con, $periodo, $fecha)
{
	...
	$sql = "create table kigsolidario2.sgas_tmp (select * from kigsolidario2.sgas_trx where terminal_date='".$fecha."' and procode='020000');".
"insert into kigsolidario2.reporte_diaria_comer (ciudad,barrio,id_tr,fecha_tr,hora_tr,apellido_nombre,nro_doc, ".
"periodo,prod_id,cantidad_kilo,codigo_proveedor,ubicacion_entrega,tipo_trans,anulado, ".
...
"); drop table kigsolidario2.sgas_tmp;";
	...
	$qret = mysqli_multi_query($con, $sql) or die("RepoGenerador_Comer() MYSQL: ".mysqli_error($con)."\n");
...
//RepoGenerador($conn, $per, $ayer);

RepoGeneradorComer($conn, $per, $ayer);
