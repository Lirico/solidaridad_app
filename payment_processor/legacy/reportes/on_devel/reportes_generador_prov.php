...
function CreateTemporal_TRX($con, $fecha)
{
	$sql = "create table kigsolidario2.sgas_tmp (select * from kigsolidario2.sgas_trx where ".
               "terminal_date = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) and procode='020000');";
...
"s1.marca_anulado ".
",s1.tarjeta, s1.direccion, s1.cod_comercio, s1.terminalid, s1.lote, s1.numero_comprobante, ".
"s1.prodid_com, s1.envace, s1.auth, ".
"s1.trunid ".
"from ( ".
...
"DATE_FORMAT(CURRENT_DATE(), '2022-07-01') as periodo, ".
...
"concat('0', cast(prd.id_normal as char(1)), '00', if(prd.id_normal=3, '90', ".
"substr(cast(prd.kgas_carga as char ),1,2) ) ) prodid_com, ".
...
"where trx.nro_tarjeta=usr.nro_tarjeta and trx.cod_comercio=comer.cod_comercio and ".
"trx.terminalid=tr.codigo_terminales and (trx.nro_tarjeta!='6063007014999900') and ".
"trx.procode!='020000' ".
"and trx.tipo_mensaje!='0400' and trx.cod_moneda = prd.cod_moneda order by trx.lote, trx.numero_comprobante".
") s1 WHERE s1.terminal_date = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) ); drop table kigsolidario2.sgas_tmp;";
